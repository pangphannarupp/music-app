...
export const PodcastBrowser: React.FC = () => {
    ...
    return (
        <div className="flex-1 overflow-y-auto p-8 h-full bg-zinc-900 scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-transparent">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                ...
                <div className="flex items-center gap-4 bg-zinc-800 p-1 rounded-full">
                    <button
                        ...
                        className={`px-4 py-1.5 rounded-full text-sm font-medium transition ${activeTab === 'khmer' ? 'bg-primary text-white shadow-lg' : 'text-zinc-400 hover:text-white'}`}
                    >
                        Khmer
                    </button>
                    <button
                        onClick={() => handleTabChange('international')}
                        className={`px-4 py-1.5 rounded-full text-sm font-medium transition ${activeTab === 'international' ? 'bg-primary text-white shadow-lg' : 'text-zinc-400 hover:text-white'}`}
                    >
                        International
                    </button>
                </div>
                <form onSubmit={handleSearch} className="relative">
                    <input
                        ...
                        className="bg-zinc-800 text-white px-4 py-2 rounded-full border border-zinc-700 focus:border-primary focus:outline-none w-full md:w-64 placeholder-zinc-500"
                    />
                </form>
            </div>
                        ...
                        {loading && podcasts.length === 0 ? (
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                                {podcasts.map((podcast) => (
                                    <div
                                        key={podcast.id}
                                        className="group cursor-pointer bg-zinc-800/50 hover:bg-zinc-800 p-4 rounded-xl transition duration-300 border border-zinc-700/50 hover:border-zinc-600"
                            ...
                            <div className="space-y-8">
                                <div className="flex flex-col md:flex-row gap-8 items-start">
                                    ...
                                    <div className="flex-1">
                                        <h1 className="text-4xl font-bold text-white mb-2">{selectedPodcast.title}</h1>
                                        <p className="text-xl text-zinc-300 mb-4">{selectedPodcast.artist}</p>
                                        <div className="flex flex-wrap gap-2 mb-4">
                                            {selectedPodcast.genres?.map(g => (
                                                <span key={g} className="px-3 py-1 rounded-full bg-zinc-800 text-xs text-zinc-300">{g}</span>
                                            ))}
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            {episodes.map((episode) => (
                                                ...
                                                >
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <h4 className={`font-semibold truncate mb-1 ${currentSong?.id === episode.id ? 'text-primary' : 'text-white'
                                                            }`}>
                                                            {episode.title}
                                                        </h4>
                                                        <p className="text-sm text-zinc-400 line-clamp-2 md:line-clamp-1">
                                                            {episode.description}
                                                        </p>
